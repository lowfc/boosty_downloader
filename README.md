...
Приложение предназначено для синхронизации (скачивания) вашего контента на boosty.to с вашим компьютером. Синхронизация может выполняться в двух режимах: для медиа и для постов.

Используя приложение, вы подтверждаете согласие с [пользовательским соглашением](https://github.com/lowfc/boosty_downloader/blob/master/assets/user_agreement.txt).
...
- Если вы переименуете, или удалите файлы, или посты - при следующей синхронизации они будут загружены снова (если не используется оптимизация file -> sync_offset_save).
- Вы можете синхронизировать контент без авторизации, но в таком случае будет недоступно скачивание закрытых, и даже некоторых открытых медиа и прикрепленных файлов. Это особенность работы boosty.to.
...
### Конфигурация
1. Переименуйте (или скопируйте) конфиг из `config.yml.example` в `config.yml`;
2. Откройте конфиг любым текстовым редактором;
3. Заполните нужные поля конфига. Ниже приведено описание всех полей:
...
| logging | logs_path              | Путь к директории логов                                                                                                     | C:\\boosty_downloader_logs                              | Нет          |
...
Запускайте с python версии 3.11, или выше.
